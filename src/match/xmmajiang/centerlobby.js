...
const Lobby = LobbyFactory({
  ...
  normalizeRule: async (rule) => {
    let specialReward = 0
    let luckyRewardList = []
    const specialRewardConfig = await RewardConfigModel.findOne({game: GameType.xmmj, type: RewardType.special}).lean()
    const luckyRewardConfig = await RewardConfigModel.find({game: GameType.xmmj, type: RewardType.lucky}).lean()
