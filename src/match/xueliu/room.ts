...
class Room extends RoomBase {
  ...
  indexOf(player) {
    console.warn("playerOrder-%s, player-%s", JSON.stringify(this.playersOrder), JSON.stringify(player));
  ...
  async forceDissolve() {
    ...
    allOverMessage.location = "xueliu.room";
  ...
  leave(player) {
    ...
    p.room = null
    this.broadcast('room/leaveReply', {ok: true, data: {playerId: p._id.toString(), roomId: this._id, index: this.indexOf(player), location: "xl.room"}})
