 ...
 */
import {ConsumeLogType, GameType, RobotStep, shopPropType, TianleErrorCode} from "@fm/common/constants";
...
const gameType: GameTypes = "guobiao"
...
class Room extends RoomBase {
  ...
  async forceDissolve() {
    ...
    allOverMessage.location = "gb.room";
  ...
  leave(player) {
    ...
    p.room = null
    this.broadcast('room/leaveReply', {ok: true, data: {playerId: p._id.toString(), roomId: this._id, location: "gb.room"}})
