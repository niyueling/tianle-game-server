...
class PlayerState implements Serializable {
  ...
  onShuffle(juShu, cards: Card[], seatIndex, juIndex, needShuffle?: boolean) {
    this.cards = cards
    ...
    this.sendMessage('game/ShuffleCards', {ok: true, data: {juShu, cards, juIndex, needShuffle: !!needShuffle }})
  }
  ...
  deposit(callback) {
    ...
    if (!this.onDeposit) {
      this.timeoutTask = setTimeout(() => {
        this.onDeposit = true;
        this.room.broadcast('game/startDepositReply', {ok: true, data: {index: this.seatIndex}})
        callback();
