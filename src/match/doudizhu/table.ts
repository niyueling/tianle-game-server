...
abstract class Table implements Serializable {
  ...
  async fapai(payload) {
    ...
    const allPlayerCards = this.cardManager.genCardForEachPlayer(this.room.isPublic, payload.cards || [], this.rule.test);
  ...
  depositForPlayerChooseMode(player: PlayerState) {
    player.deposit(async () => {
      ...
      // console.warn("mustCallLandlord %s jokerCount %s twoCount %s landlordIndex %s", this.rule.mustCallLandlord, jokerCount, twoCount, index);
